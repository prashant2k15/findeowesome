...
Your own imports  ─┘                                              │
                          ...
                          ┌───────────────────────────────────────┼──────────────┐
                          ▼                                       ▼              ▼
                   master database                      CSV exports        Telegram digest
                    (PostgreSQL)                     + web dashboard        + failure alerts
```

**Discovery and tracking only.** It finds and qualifies places where links can
...
app/
  ...
  notify/       telegram.py
  db/           models.py  session.py  repo.py    # all writes go through repo.py
  api/          main.py  templates/dashboard.html
  jobs.py  worker.py  cli.py  config.py
config/         footprints.json  seed_sources.yaml  searxng/settings.yml
...
- Domain metrics enrichment (Moz/Majestic/Ahrefs API when a key is present) to
  sort by real authority rather than page signals alone
- Language/geo detection per opportunity
- Duplicate-network detection (same footer, same owner, PBN-ish clusters)
- Optional submission-assist: pre-filled form data and a queue you approve
  manually, rather than blind auto-posting
